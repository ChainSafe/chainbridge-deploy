...
func NewClient(url string, privateKey string, bridge, erc20 common.Address, erc20Handler common.Address, log log.Logger) (*Client, error) {
...
func (c *Client) CreateFungibleDeposit(amount *big.Int, recipient string, rId msg.ResourceId, destId msg.ChainId) (msg.Nonce, *big.Int, error) {
	...
	err := utils.Erc20Approve(c.client, c.erc20, c.erc20Handler, amount)
	if err != nil {
		return 0, nil, err
	}
	...
	if receipt.Status != 1 {
		return 0, nil, fmt.Errorf("fungible deposit transaction failed. Tx: %s", receipt.TxHash.Hex())
	}
	...
	return nonce, receipt.BlockNumber, nil
}
...
func (c *Client) VerifyFungibleProposal(amount *big.Int, recipient string, source msg.ChainId, nonce msg.Nonce) error {
	...
	startBlock, err := c.client.Client.BlockByNumber(context.Background(), nil)
	if err != nil {
		return err
	}
...
func (c *Client) WaitForProposalEvent(startBlock *big.Int, nonce msg.Nonce, source msg.ChainId, status utils.ProposalStatus) (common.Hash, error) {
