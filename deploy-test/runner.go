...
func Start(cfg *Config) ([]TestFailure, error) {
	...
	if cfg.Source.Type == EthereumType {
		cfg.Source.Client, err = ethereum.NewClient(cfg.Source.Endpoint, cfg.PrivateKey, cfg.Source.Bridge, cfg.Source.Erc20, log.New("chain", "source"))
	} else if cfg.Source.Type == SubstrateType {
		cfg.Source.Client, err = substrate.NewClient(cfg.Source.Endpoint, cfg.PrivateKey)
	} else {
	...
	if cfg.Destination.Type == EthereumType {
		cfg.Destination.Client, err = ethereum.NewClient(cfg.Destination.Endpoint, cfg.PrivateKey, cfg.Destination.Bridge, cfg.Destination.Erc20, log.New("chain", "dest"))
	} else if cfg.Destination.Type == SubstrateType {
		cfg.Destination.Client, err = substrate.NewClient(cfg.Destination.Endpoint, cfg.PrivateKey)
	} else {
		return nil, fmt.Errorf("unrecognized chain type: %s", cfg.Source.Type)
	}
